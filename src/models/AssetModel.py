...
class AssetModel(BaseDataModel):
    ...
    async def create_asset(self, asset: Asset):
        """Insert or update an asset (upsert by asset_id to avoid duplicates)."""
        doc = asset.dict(by_alias=True, exclude_unset=True)
        doc.pop("_id", None)
    ...
    async def get_all_assets(self, project_id: str, asset_type: str):
        ...
        return [Asset(**{k: v for k, v in asset.items() if k != "_id"}) for asset in result]
