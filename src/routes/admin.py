...
async def get_system_stats(request: Request):
    ...
    # Chunks — نعد عبر المشاريع الموجودة
    total_chunks = 0
    try:
        db = request.app.db
        if db is not None:
            # كل project عنده collection خاصة بـ chunks
            all_collections = await db.list_collection_names()
            chunk_collections = [c for c in all_collections if c.startswith("chunks_")]
            for col_name in chunk_collections:
