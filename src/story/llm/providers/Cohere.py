...
class CoHereProvider(LLMInference):
    ...
    async def embed_text(self, text, document_type: str = None):
        if not self.client:
            self.logger.error("CoHere client was not set")
            return None
    ...
    async def generate_story(self, prompt: str, max_tokens: int = None,
                       temperature: float = None, chat_history: list = None) -> str:
        return await self.generate_text(prompt, chat_history=chat_history or [],
