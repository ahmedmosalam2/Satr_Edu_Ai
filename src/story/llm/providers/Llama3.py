...
from ..LLMinfernce import LLMInference


class LlamaProvider(LLMInference):
    ...
    ,default_embedding_temperature:float=0.1):
        ...
        self.client=AutoModelForCausalLM.from_pretrained(api_key)
        self.tokenizer=AutoTokenizer.from_pretrained(api_url)
    ...
    def generate_embedding(self,text:str)->list[float]:
        return self.embedd_text(text, LLMEnums.DocumentTypeEnum.DOCUMENT.value)

    def generate_story(self,prompt:str,max_tokens:int=None,temperature:float=None,chat_history:list[dict]=None)->str:
        return self.generate_text(prompt, max_tokens=max_tokens, temperature=temperature, chat_history=chat_history)
