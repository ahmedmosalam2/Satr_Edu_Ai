...
class OllamaProvider(LLMInference):
    ...
    def generate_embedding(self, text: str) -> list:
        # Fallback for anything that still calls this directly
        res = self.embed_text(text)
        return res[0] if res else None

    def embed_text(self, text, document_type=None):
        """Supports both single string and list of strings, natively batching if possible."""
        if not self.embedding_model_id:
            self.logger.error("Ollama: embedding model not set")
            return None

        is_list = isinstance(text, list)
        processed_input = [self.process_text(t) for t in text] if is_list else self.process_text(text)
        ...
        for endpoint, key in [("/api/embed", "embeddings"), ("/api/embeddings", "embedding")]:
            try:
                ...
                if endpoint == "/api/embed":
                    ...
                    if is_list:
                        # Fallback for old Ollama versions that don't support /api/embed batching
                        self.logger.warning("Ollama /api/embed failed, falling back to sequential /api/embeddings (this will be slow)")
                        results = []
                        for t in processed_input:
                            payload["prompt"] = t
                            resp = requests.post(f"{self.base_url}{endpoint}", json=payload, timeout=60)
                            if resp.status_code == 200:
                                results.append(resp.json().get(key))
                        return results
                    else:
                        payload["prompt"] = processed_input

                # Batch requests might take longer, increase timeout
                resp = requests.post(f"{self.base_url}{endpoint}", json=payload, timeout=300)
                if resp.status_code == 200:
                    data = resp.json()
                    result = data.get(key)
                    if result:
                        if endpoint == "/api/embed":
                            # /api/embed ALWAYS returns a 2D array: {"embeddings": [[...], ...]}
                            return result
                        else:
                            # /api/embeddings returns a 1D array for single prompt: {"embedding": [...]}
                            return [result]
            except Exception as e:
                self.logger.warning(f"Ollama {endpoint} failed: {e}")
                
        self.logger.error(f"Ollama: all embedding endpoints failed. Is the model '{self.embedding_model_id}' pulled?")
        return None